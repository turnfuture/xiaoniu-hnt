      ...
      <div :class="$style['pro-right']">
        <ul :class="$style['pro-content']">
          ...
          <li v-for="item in list" :key="item.id">
            <router-link :to="{ name: 'details', params: { type: 'product', id: item.id} }">
              <div :class="$style.pic">
...
export default {
  ...
  data() {
    return {
      banner: require('../assets/img/product.png'),
      asides: [{
        title: '',
        content: ''
      }],
  ...
  methods: {
    init() {
      // 初始化轮播图
      this.$dataPost('/api/cms/achieve/type', {}, (res) => {
        if (res.code === 0) {
